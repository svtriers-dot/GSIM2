...
> Поддомен: **toc.tesstech.ru** (поправить в setup.sh при необходимости)
...
```bash
ssh ubuntu@toc.tesstech.ru   # или по IP пока DNS не пропагирован
...
После того как DNS A-запись для `toc.tesstech.ru` ведёт на IP VM:

```bash
sudo apt-get install -y certbot python3-certbot-nginx
sudo certbot --nginx -d toc.tesstech.ru \
