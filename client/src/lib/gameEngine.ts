...
export class GoldrattEngine {
  ...
  private initBuffers() {
    ...
    // Стартовая незавершённая продукция (как в исходном дизайне): даёт разгон
    // потока в день 1 на нескольких участках. Это бесплатный стартовый актив
    // фабрики (касса не уменьшается, сырьё за неё не списывается). Следствие:
    // реальный потолок прибыли выше «холодного» (≈11 500 ₽ против 10 000 ₽),
    // т.к. ~75 единиц сырья (1 500 ₽) уже воплощены в стартовых запасах.
    this.buffers['B3_shared'] = 25; // 25 готовых сборок B3 (вход для A5 и C5)
    this.buffers['E2_out'] = 15;    // 15 полуфабрикатов D-цепочки
    this.buffers['F3_out'] = 10;    // 10 полуфабрикатов F-цепочки
  }
